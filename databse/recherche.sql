...
CREATE TABLE `recherche` (
  `rechercheId` bigint NOT NULL AUTO_INCREMENT,
  `titre` varchar(255) DEFAULT NULL,
  `dateCreation` date DEFAULT NULL,
  `dateUpdate` date DEFAULT NULL,
  `docId` bigint DEFAULT NULL,
  `statu` enum('en cours','soutenu') DEFAULT NULL,
  PRIMARY KEY (`rechercheId`),
  KEY `docId` (`docId`),
  CONSTRAINT `recherche_ibfk_1` FOREIGN KEY (`docId`) REFERENCES `doctorant` (`docId`) ON DELETE SET NULL
